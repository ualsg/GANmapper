...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    try:
        os.makedirs(image_dir+'/real')
        os.makedirs(image_dir+'/fake')
        os.makedirs(image_dir+'/realA')
    except:
        pass
...
def save_images_predict(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    name = image_path[0][-18:] #find slippymap dir from source
    webpage.add_header(name)
    ims, txts, links = [], [], []
    ...
    for label, im_data in visuals.items():
        im = util.tensor2im(im_data)
        if label == 'real':
            image_name = 'input\\' + str(name)
        if label == 'fake':
            image_name = 'fake\\' + str(name)
        save_path = os.path.join(image_dir, image_name)
        dir_path = os.path.dirname(save_path)
        Path(dir_path).mkdir(parents=True, exist_ok=True) # create subdirs
